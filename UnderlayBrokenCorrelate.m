...
%         end
    ...
    FreqOS(index) = flag1;
    if index >500
        mean_shcorr = mean(abs_SynCorr(index-500:index));
    else
        mean_shcorr = mean(abs_SynCorr(1:index));
    end
    if rem(index, pnSeqLen*(short_seq_rep-1)) == 0
        ShortPkThres = 0.7*ShortPkThres +0.3*mean_shcorr;
    end 
    ShortThreshold(index) = ShortPkThres;
    %%%% long threshold update
    if flag2>LongSeqThres
        if (flag2>LongThresFlag)
            LongThresFlag = flag2;
        end
        for rep = 1:short_seq_rep
...
%             ShortPkThres = 0.7*ShortPkThres +0.3*newThres*0.9;
        ...
        LongSeqThres = 0.7*LongSeqThres + 0.3*tempThres*0.75;
        LongThresFlag = 0;
        %disp(LongSeqThres);
    end
